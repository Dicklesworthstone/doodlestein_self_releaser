...
_xwb_run() {
    ...
    set +m
    local cwd="$1" log="$2" seconds="$3" pid rc=0
    shift 3
    _xwb_cancel() {
        trap '' HUP INT TERM
...
_xwb_build() {
    ...
    while (($#)); do
        ...
        case "$1" in
            ...
            --manifest|--project|--run-dir|--bin|--package|--cache-dir|--cargo-cache|--timeout)
                [[ $# -ge 2 && -n "$2" && "$2" != --* ]] || return 4
                case "$1" in
    ...
    _xwt_require || return $?
    local tool parent plan view entry name path key rustc target=aarch64-pc-windows-msvc manifest_hash lock_hash child=0 exit_trap
    ...
    # Existing DSR source staging owns ancestor isolation. Refuse an unstaged
    # project here rather than silently inheriting unrelated Cargo configuration.
    ...
    # Do not allow a local JSON target to shadow cargo-xwin's built-in target.
    ...
    local rc=0
    _xwb_run "$project" "$run/build.log" "$seconds" env -i "${environment[@]}" "${command[@]}" &
    child=$!
    wait "$child" || rc=$?
    child=0
    ...
    cp -- "$path" "$run/artifacts/$binary.exe" || return 1
    xwin_validate_arm64_pe "$run/artifacts/$binary.exe" > "$run/artifact.json" || return $?
    jq -cn --arg run "$run" --arg project "$project" --arg bin "$binary" --arg key "$key" \
        ...
        '{schema_version:1,kind:"dsr-xwin-build",status:"verified",exit_code:0,
          ...
          manifest_sha256:$key,source_inputs:$source[0],artifact:($artifact[0]+{path:($run+"/artifacts/"+$bin+".exe")}),
